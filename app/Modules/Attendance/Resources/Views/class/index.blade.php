    ...
    <div class="container">
        <div class="row">
            <div class="panel panel-default">
                <div class="panel-heading">Class Rooms</div>
                <div class="panel-body">
                    <div class="col-md-6">
                        <a class="btn btn-success" href="{{ route('class.create') }}"> Create Class Room</a>
                        <input type="button" class="btn btn-danger" id="deleteMenu" value="Delete Class Room">
                        {!! Form::open([ 'method'  => 'delete','id' => 'deletePageForm', 'route' => [ 'class.destroy', 0] ]) !!}
    ...
    <script type="text/javascript">
        jQuery(function($) {
            $('#cmspageTable').DataTable({
                ...
                info:false,
                order: [[ 1, "asc" ]],
                ajax: '{!! url('attendance/data-table-class') !!}',
                columns: [
                    { data: 'check', name: '', width:10, orderable:false, searchable:false },
                    { data: 'name', name: 'name' },
                    { data: 'iot_id', name: 'iot_id' },
                ],
                aoColumnDefs: [{
                    "aTargets": [ 1 ], // Column to target
                     "mRender": function ( data, type, full ) {
                         if (full.name != "")
                         {
                             //returnStr =  '<a href="{{ url('attendance/class')}}/'+full.id+'/edit">'+full.name+'</a>';
                             returnStr =  full.name;
