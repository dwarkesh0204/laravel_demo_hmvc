        ...
        <div class="row">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="col-md-6">
                        <a class="btn btn-success" href="{{ route('semester.create') }}"> Create Semester</a>
                        <input type="button" class="btn btn-danger" id="deleteMenu" value="Delete Semester">
                        {!! Form::open([ 'method'  => 'delete','id' => 'deletePageForm', 'route' => [ 'semester.destroy', 0] ]) !!}
    ...
    <script type="text/javascript">
        jQuery(function($) {
            $('#cmspageTable').DataTable({
                ...
                info:false,
                order: [[ 1, "desc" ]],
                ajax: '{!! url('attendance/data-table-semester') !!}',
                columns: [
                    { data: 'check', name: '', width:10, orderable:false, searchable:false },
                    { data: 'name', name: 'semester.name' },
                    { data: 'course_name', name: 'course.name' },
                    { data: 'description', name: 'semester.description' },
                ],
                aoColumnDefs: [{
                    "aTargets": [ 1 ], // Column to target
                     "mRender": function ( data, type, full ) {
                         if (full.name != "")
                         {
                             returnStr =  '<a href="{{ url('attendance/semester')}}/'+full.id+'/edit">'+full.name+'</a>';
                         }
                         else
                         {
                            returnStr = '---';
