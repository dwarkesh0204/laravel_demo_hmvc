...
use App\Modules\Events\Models\Beacon;
use App\Modules\Events\Models\Calibrator;
use App\Modules\Events\Models\Gateway;
use App\Modules\Events\Models\Nodes;
use App\Modules\Events\Models\Floorplan;
use App\Modules\Events\Models\Poi;
use App\Modules\Events\Models\RssiLog;
use App\Modules\Events\Models\NodeRelation;
use Session;
...
class FloorplanController extends Controller
{
    ...
    {
        return view('events::floorplan.index');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($FloorplanID) {
            ...
            return redirect()->route('floorplans.index')->with('flash_message', 'Floorplan created successfully');
        } else {
    ...
    public function edit($id)
    {
        ...
        return view('events::floorplan.edit', compact('FloorplanData', 'AllBeacons', 'id', 'AreasArray', 'FloorplanNodes', 'ImageWidth', 'ImageHeight', 'AssignedNodes', 'FloorNodes_dropdown','FloorBeacons','FloorCalibrators','FloorPois','FloorGateway'));
    }
    ...
    public function DeleteFloorplan($FloorplanIDs)
    {
        ...
        foreach ($FloorplanIDs as $key => $FloorplanID) {
            ...
            if(count($deviceids))
            {
                $pid = \Session::get('events_conn_id');
    ...
    public function ajaxInsertNodesPathsData($post)
    {
        ...
        for($b=0; $b<count($post['beacons']); $b++)
        {
            if($post['beacons'][$b]['temp'])
            {
                ...
                if(count($bm_beacon))
                {
                    $event_beacon = new Beacon();
                    $event_beacon->name         = $bm_beacon->name;
                    $event_beacon->type         = $bm_beacon->type;
                    $event_beacon->floorplan_id = $post['floorPlan'];
                    $event_beacon->major_id     = $bm_beacon->major_id;
                    $event_beacon->minor_id     = $bm_beacon->minor_id;
                    $event_beacon->uuid         = $bm_beacon->uuid;
                    $event_beacon->macid        = $bm_beacon->macid;
                    $event_beacon->beacon_X     = $post['beacons'][$b]['x'];
                    $event_beacon->beacon_Y     = $post['beacons'][$b]['y'];
                    $event_beacon->display_X    = $post['beacons'][$b]['x']*$x_ratio;
                    $event_beacon->display_Y    = $post['beacons'][$b]['y']*$y_ratio;
                    $event_beacon->status       = $bm_beacon->status;
                    $event_beacon->created_at   = Carbon::now();
                    $event_beacon->updated_at   = Carbon::now();
                    $event_beacon->save();
    ...
    public function ajaxAddBeaconData($request)
    {
        ...
        if(count($Beacon)>0)
        {
            ...
            /*$event_beacon = new Beacon();
            $event_beacon->name         = $Beacon->name;
            $event_beacon->type         = $Beacon->type;
            $event_beacon->floorplan_id = $request->floorPlan;
            //$event_beacon->project_id   = $Beacon->project_id;
            $event_beacon->major_id     = $Beacon->major_id;
            $event_beacon->minor_id     = $Beacon->minor_id;
            $event_beacon->uuid         = $Beacon->uuid;
            $event_beacon->macid        = $Beacon->macid;
            $event_beacon->beacon_X     = $Beacon->beacon_X;
            $event_beacon->beacon_Y     = $Beacon->beacon_Y;
            $event_beacon->status       = $Beacon->status;
            $event_beacon->created_at   = Carbon::now();
            $event_beacon->updated_at   = Carbon::now();
            $event_beacon->save();*/

            //$beacon_floorplan = $Beacon->where('id',$Beacon->id)->update(['floorplan_id'=>$request->floorPlan, 'project_id'=>$pid]);
        }
        else
        {
