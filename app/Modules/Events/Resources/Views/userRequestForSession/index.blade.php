    ...
    <div class="container">
        <div class="row">
            <div class="panel panel-default">
                <div class="panel-heading">User request for session</div>
                    <div class="panel-body">
                        <div class="col-md-6">
                            <input type="button" class="btn btn-danger" id="deletePage" value="Delete">
                            {!! Form::open(['method' => 'DELETE','route' => ['UserRequestForSession.destroy',1],'style'=>'display:inline', 'id' => 'deletePageForm']) !!}
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered"
                        ...
                        <tbody>
                        @foreach($userRequestForSession as $user_request_detail)
                            <tr>
                                <td><input type="checkbox" name="select_one" value="{{$user_request_detail['id']}}"
                                           class="select_one">
                                </td>
                                <td>{{$user_request_detail->user_request_for_session_user['name']}}</td>
                                <td>{{$user_request_detail->user_request_for_session_session['name']}}</td>
                                <td align="center">
                                    @if($user_request_detail->status == 1)
                                       {!! Form::open(['route' => ['UserRequestForSession.changeStatus', $user_request_detail['id']],'style'=>'display:inline']) !!}
