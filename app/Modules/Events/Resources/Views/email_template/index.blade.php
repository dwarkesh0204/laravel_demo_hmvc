...
@section('content')
        ...
        <div class="row">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="col-md-6">
                        <a class="btn btn-success" href="{{ route('email_template.create') }}"> Create Email Template </a>

                        <input type="button" class="btn btn-danger" id="deleteMenu" value="Delete Email Template">
                        {!! Form::open(['method' => 'DELETE','route' => ['email_template.destroy', 1],'style'=>'display:inline', 'id' => 'deletePageForm']) !!}
                        <input type="hidden" name="emailtmpIDs" id="emailtmp" value="">
                        {!! Form::close() !!}
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered"
                        ...
                        @foreach($items as $key => $item)
                            <tr>
                                ...
                                <td><a href="{{ route('email_template.edit',$item['id']) }}">{{ $item['title'] }}</a></td>
                                <td>{{ $item['created_at'] }}</td>
                                <td>{{ $item['updated_at'] }}</td>
                                <td align="center">
                                    @if($item['status'] == 1)
                                        {!! Form::open(['route' => ['email_template.changeStatus', $item['id']],'style'=>'display:inline']) !!}
                                        <a class="btn btn-micro active" href="javascript:void(0);" onclick="" title=""
                                           data-original-title="Unpublish Item">
                                            <span class="glyphicon glyphicon-star" aria-hidden="false"
                                    ...
                                    @else
                                        ...
                                        <a class="btn btn-micro active" href="javascript:void(0);" onclick="" title=""
                                           data-original-title="Publish Item">
                                            <span class="glyphicon glyphicon-star-empty" aria-hidden="false"
    ...
    jQuery(function($) {
        $('#menuTable').DataTable({
            ...
            info:false,
            order: [[ 2, "desc" ]],
            ajax: '{!! url('events/data-table-email_template') !!}',
            columns: [
            ...
            aoColumnDefs: [{
                "aTargets": [ 2 ], // Column to target
                 "mRender": function ( data, type, full ) {
                     if (full.title != "")
                     {
                         returnStr =  '<a href="{{ url('events/email_template')}}/'+full.id+'/edit">'+full.title+'</a>';
                ...
                },
                {
                    ...
                    "mRender": function ( data, type, full ) {
                        if (full.status == 1)
                        {
                            returnStr =  '<span class="glyphicon glyphicon-star" aria-hidden="false" style="color: green; font-size: 15px;"></span>';
                        }
                        else
                        {
                            returnStr =  '<span class="glyphicon glyphicon-star-empty" aria-hidden="false" style="color: red; font-size: 15px;"></span>';
                        }
                    return returnStr;
                    }
                }
    ...
    jQuery(document).ready(function ($) {
        ...
        jQuery('td input[name="select_one"]').on('click', function (e) {
            if (this.checked) {
                ...
                jQuery('#emailtmp').val(CmsPageIDs);
            } else {
                var pageID = jQuery(this).val();
        ...
        jQuery('th input[name="select_all"]').on('click', function (e) {
            if (this.checked) {
                ...
                jQuery('#emailtmp').val(allPageIDs);
            } else {
    ...
    jQuery('#deleteMenu').click(function (event) {
        var deleteId = jQuery('#emailtmp').val();
        if (deleteId != '') {
